...
- **Current War card fills the gap**: The docs/concept/05 spec had the current-war summary only as a navigation strip at the bottom. Promoting it to a card in the stats row gives it more prominence and balances the 3-column layout. The bottom NavSummaries component is kept for the capital raid weekend strip + war link.
