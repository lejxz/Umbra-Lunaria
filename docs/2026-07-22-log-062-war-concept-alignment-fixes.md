...
## Summary of Session
Audit of the War Center against docs/concept/07 found three missing clauses, one misleading label, and a tradeoff in the roster redesign (defense-only compact row dropped the offense/urgency cues). Fixed all four gaps and added a prep/battle toggle so the roster's right column shows base-state during preparation and attacks (offense + no-attack urgency) during battle — restoring every docs/concept/07 roster clause without losing the defense view.
...
- **"Attacks" mode** (new): shows the member's best attack (★★☆ + %) and an urgency badge — red `0/N` (no attacks used, with shield icon), amber `N/N` (attacks remaining), or emerald `✓ N/N` (done). Restores docs/concept/07 §"Roster and attack status" #3 (attacks used vs allowed), #4 (best stars/destruction), and #5 (prominent no-attack/attacks-left state).
...
## Next Action
All docs/concept/07 gaps closed. The War Center now satisfies every clause of the spec. Continue to Step 1.5 — Capital (docs/concept/08).
