...
export default async function DashboardPage() {
  ...
  const warWins = clan?.warWins;
  const warTies = clan?.warTies;
  const warLosses = clan?.warLosses;
  const hasCompleteWarRecord = typeof warWins === "number" && typeof warTies === "number" && typeof warLosses === "number";
  const winTotal = hasCompleteWarRecord ? warWins + warTies + warLosses : 0;
  const winRate = hasCompleteWarRecord && winTotal > 0 ? `${Math.round((warWins / winTotal) * 100)}%` : undefined;
  return <div className="mx-auto max-w-7xl p-5 sm:p-8"><header className="mb-8 flex flex-wrap items-end justify-between gap-5"><div><p className="mb-2 font-mono text-xs uppercase tracking-[0.18em] text-umbra-purple">Command center</p><h1 className="font-display text-3xl font-bold tracking-wide text-umbra-lilac sm:text-4xl">{clan?.name ?? "Umbra Lunaria"}</h1><p className="mt-2 text-sm text-umbra-muted">Dashboard · {clanConfig.clanTag}</p></div><div className="rounded-full border border-umbra-purple/20 bg-umbra-purple/10 px-4 py-2 font-mono text-xs text-umbra-lilac">{clan?.lastPolledAt ? `Polled ${clan.lastPolledAt.toLocaleString()}` : "Awaiting first poll"}</div></header>
    ...
    <section className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Clan level" value={clan?.clanLevel ?? "—"} /><StatCard label="War wins" value={warWins ?? "—"} trend={winRate ? `${winRate} all-time win rate` : undefined} /><StatCard label="Members" value={members.length} /><StatCard label="Capital hall" value={clan?.capitalHallLevel ?? "—"} /></section>
    <section className="mt-6"><DonationPanel data={{ "24h": donation24, "7d": donation7, "30d": donation30 }} /></section>
    <section className="mt-5 grid gap-5 lg:grid-cols-2"><ActivityPanel data={activity} /><ClanLog entries={recentLog.map((entry) => ({ ...entry, date: entry.date.toISOString() }))} /></section>
