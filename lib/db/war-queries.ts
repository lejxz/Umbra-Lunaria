...
function parseWarSnapshot(row: typeof wars.$inferSelect): {
  ...
  // Merge both clans' attacks into one list.
  const allAttacks: RawAttack[] = [
    ...(snap.clan.members ?? []).flatMap((m) => m.attacks ?? []),
    ...(snap.opponent.members ?? []).flatMap((m) => m.attacks ?? []),
  ];

  const clanSide = buildClanSide(snap.clan, true, teamSize, attacksPerMember, allAttacks);
...
function buildClanSide(
  ...
  return {
    ...
    members: (side.members ?? [])
      ...
      .map((m) => buildRosterMember(m, isOwnClan, attacksPerMember, allAttacks)),
  };
}
...
function buildRosterMember(
  ...
  const defenseAttacks = allAttacks.filter((a) => a.defenderTag === m.tag);
