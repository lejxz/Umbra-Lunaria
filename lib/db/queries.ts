...
export async function getDonationLeaderboard(
  ...
  const topDonors: DonationLeaderboardEntry[] = donorEntries
    ...
    .map((e, i) => ({
      ...
      leagueTier: (memberMap.get(e.playerTag)?.leagueTier as { name: string; iconUrls?: ClanBadgeUrls }) ?? null,
      total: e.given,
      rank: i + 1,
    }));
...
export async function getDashboardWarSummary(): Promise<WarSummaryView> {
  ...
  return {
    ...
    opponentBadgeUrls: currentWar.opponentBadgeUrls as ClanBadgeUrls | null,
